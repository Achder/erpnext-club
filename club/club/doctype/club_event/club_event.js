...
const utils = {
	entrance: {
		...
		count_guests: async (event) => {
			const response = await frappe.call({
				method: 'club.club.doctype.club_event.club_event.count_entrances',
				args: { event }
			})
			return response['message'][0][0]
		},
	},
...
const balance = async (frm) => {
	...
	const calc = {
		data: {
			bar_per_capita: async (total_guests, revenue_bar) => {
				return (total_guests === 0) ? 0 : revenue_bar / total_guests
			}
		},
		costs: {
			shift: async (shifts) => {
		...
		revenue: {
			entrance: async (event) => {
				const response = await frappe.call({
					method: 'club.club.doctype.club_event.club_event.get_entrance_sales',
					args: { event }
				})
				return response['message'][0][0]
			},
			bar: async (event) => {
				const response = await frappe.call({
					method: 'club.club.doctype.club_event.club_event.get_bar_sales',
					args: { event }
				})
				return response['message'][0][0]
			},
	...
	const revenue = {
		entrance: await calc.revenue.entrance(name),
		bar: await calc.revenue.bar(name)
	}
	...
	const total_guests = await utils.entrance.count_guests(name)
	const data = {
		total_guests,
		bar_per_capita: await calc.data.bar_per_capita(total_guests, revenue.bar)
	}

	const costs_total = costs.booking + costs.drinks + costs.staff
	const revenue_total = revenue.bar + revenue.entrance

	frm.set_value('balance_actual_guests', data.total_guests)
